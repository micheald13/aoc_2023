...
fn check_for_winners_and_add(cards: Vec<LotteryCard>) {
    for (index, card) in cards.iter().enumerate() {
        if card.matches > 0 {
            for next_index in index + 1..index + 1 + card.matches {
                if let Some(next_card) = cards.get(next_index) {
                    next_card.copies += 1;
                }
            }
        }
...
fn solve_part2(input: &str) -> usize {
    let lottery_cards: Vec<LotteryCard> = input
        .lines()
